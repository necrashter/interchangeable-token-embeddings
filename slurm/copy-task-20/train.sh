...
MODEL_NAME=$1
mdirname="copy-20"
full_model_path="~/deepltl/models/$mdirname/$MODEL_NAME"

PARAMS=(
	# --num-heads=4 --d-embed-enc=64 --d-ff=64 --num-layers=2  # s
	--num-heads=8 --d-embed-enc=128 --d-ff=128 --num-layers=6  # m
	# Train settings
	--epochs=1
	# Vocab
...
if [[ "$first_group" == "0000" ]]; then
	PARAMS+=(--ds-name=cpy-20-80-160ap)
	PARAMS+=(--val-max-samples=10000)
elif [[ "$first_group" == "base" ]]; then
	PARAMS+=(--ds-name=cpy-20-80-20ap)
	PARAMS+=(--val-max-samples=10000)
else
	PARAMS+=(--ds-name=cpy-20-80-20ap)
	PARAMS+=(--val-max-samples=10000)
	...
	if [[ "$letter_part" == "d" ]]; then
		...
		PARAMS+=(--dynamic-aps)
		PARAMS+=(--ap_embed=diagbor_no_check)
		PARAMS+=(--d_ap="$number_part")
...
export MODEL_PATH="models/$mdirname/$MODEL_NAME"
