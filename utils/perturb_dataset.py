...
def perturb_dataset(
    ...
    with open(source_filename, 'r') as file:  # expect formula\ntrace\n format
        for formula in file:
            ...
            formula_aps = list(dict.fromkeys([i for i in formula if i.islower()]))
            trace_aps = list(dict.fromkeys([i for i in trace if i.islower()]))
            ...
            lookup = {k: aps[i] for i, k in enumerate(trace_aps)}
